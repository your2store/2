...
function calculateItemPrice(item) {
    ...
    if (item.selectedAddons.netType && addonPrices.net[item.selectedAddons.netType]) {
        // Correctly calculate based on charging "per 0.5m²" unit, rounding up.
...
function addItem(manualData = null) {
    ...
    if (manualData) {
        newItem = {
            ...
            h: manualData.h,
            w: manualData.w,
            itemCodePrefix: manualData.itemCodePrefix || "", // Added to store prefix from bulk add
            itemNotes: manualData.itemNotes || "", // Renamed label to itemNotes for clarity
            selectedAddons: {},
        ...
        newItem = {
            ...
            h: height,
            w: width,
            itemCodePrefix: "", // Manual items don't have a code prefix by default
            itemNotes: "",
            selectedAddons: {
...
function processPastedText() {
    ...
    alert(alertMessage);
    document.getElementById("bulkAddText").value = ""; // Clear the text area after processing
}
...
function saveItemEdit(id) {
    ...
    if (item) {
        ...
        // Input validation for edited fields
